...
public class Plugin : BaseUnityPlugin
{
    ...
    private void OnGUI()
    {
        ...
        menuRect = GUI.Window(0, menuRect, (id) =>
        {
            ...
            if (GUILayout.Button("Unlock All Guys", GUILayout.Height(menuControlHeight)))
            {
            ...
            if (GUILayout.Button("Set Current Guy To Lover", GUILayout.Height(menuControlHeight)))
            {
            ...
            if (GUILayout.Button("Set All Guys To Lover", GUILayout.Height(menuControlHeight)))
            {
                Balance.GirlName newestGirl = Enum.GetValues(typeof(Balance.GirlName))
                    .Cast<Balance.GirlName>()
                    .Where(g => (int)g < 1000)
            ...
            if (GUILayout.Button("Meet Current Guy Heart Requirement", GUILayout.Height(menuControlHeight)))
            {
                if (Girls.CurrentGirl == null)
                {
                    Logger.LogWarning("No current guy is selected.");
                }
                else
                {
            ...
            if (GUILayout.Button("Meet All Current Guy Requirements", GUILayout.Height(menuControlHeight)))
            {
                if (Girls.CurrentGirl == null)
                {
                    Logger.LogWarning("No current guy is selected.");
        ...
        if (bShowConfirmPopup)
        {
            ...
            popupRect = GUI.Window(1, popupRect, (id) =>
            {
                ...
                GUILayout.Label("This will unlock all items in the game including all guys and premium content.\nYou    must save and reload for this to take effect.\n\nAre you sure?");
...
{
    [HarmonyPostfix]
    static void Postfix(ref bool __result, BlayFapIntegration.ServerItem item)
    {
        if (!__result && Plugin.bUnlockAllItems.Value)
        {
            if (Plugin.bExtraDebugLogs)
                Plugin.Logger.LogInfo($"Pretending player has item {item}");
...
{
    [HarmonyPostfix]
    static void Postfix(ref bool __result, int pinupPage, int image)
    {
        if (!__result && Plugin.bShowAllPinups.Value)
        {
            if (Plugin.bExtraDebugLogs)
                Plugin.Logger.LogInfo($"Pretending pinup {pinupPage}-{image} is unlocked");
            __result = true;
        }   
    }
}
...
public class Gift_Init_Patch
{
    ...
    static void Postfix(Gift __instance)
    {
        ...
        if (!TryGetGiftOutfitType(__instance, out OutfitModel outfitModel))
            return;
        ...
        try
        {
            if (!TryUnlockOutfitForGirl(Girls.CurrentGirl, outfitModel))
                return;
            ...
            if (Plugin.bExtraDebugLogs)
            {
                Plugin.Logger.LogInfo($"Unlocked outfit model for {Enum.GetName(typeof(Balance.GirlName), Girls.CurrentGirl.GirlName)}");
            }
        }
        catch (Exception ex)
        {
    ...
    private static bool TryGetGiftOutfitType(Gift gift, out OutfitModel outfitModel)
    {
        outfitModel = null;
        if (gift == null)
            return false;
    ...
    private static bool TryUnlockOutfitForGirl(Girl girl, OutfitModel outfitModel)
    {
